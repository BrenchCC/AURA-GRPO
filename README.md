...
剧情背景生成包含两次 LLM 调用。第一次是视觉 LLM 调用，用来把多集视频帧压缩成逐集概要；第二次是文本 LLM 调用，用来把逐集概要合并成短剧级背景。
...
人物信息生成只调用一次视觉 LLM，但它的输入比剧情概要更强调帧定位。每张图片进入消息前，都会先追加一条文本标记：`chapter_rank: N, frame_index: K`，再追加对应的 base64 图片。这里的 `frame_index` 不是原视频里的 `frame_idx`，而是人物分支在当前章节内重新编号的临时索引；代码同时维护 `Chapter2Idx2Path[chapter_rank][frame_index]`，用于把 LLM 返回的标志帧映射回真实图片路径。
  ...
  "characters": [
    {
      ...
      "role_title": "男主，家族继承人",
      "iconic_frames": [
        {"chapter_rank": 1, "frame_idx": "2"},
        {"chapter_rank": 3, "frame_idx": "8"}
      ]
    }
  ]
}
...
这里要求 `name` 尽量使用剧中人名；如果画面或字幕没有明确姓名，配角可以用与主角关系绑定的称谓，例如「张旭东母亲」。`role_title` 只保留身份、关系或叙事功能，不扩写剧情。`iconic_frames` 的作用是把人物信息绑定到可追溯画面上，所以 prompt 明确要求优先选择人物清晰可见、正脸、无遮挡的帧；如果帧中还有字幕或场景提示，则更适合作为标志帧。
...
训练样本使用 scene 级视觉证据池，而不是把短剧的全部原始视频帧直接送入模型。每个 scene 先按照时长选择代表帧：长 scene 取首帧和尾帧，短 scene 取中间帧；随后额外保留最多 3 位主要人物的角色证据帧，每位人物最多 5 帧，因此人物角色帧总数不超过 15 帧。合并后按照图片路径或原始帧索引去重，避免同一张图片同时作为 scene 证据和人物角色证据重复计算。

单条训练输入的视觉帧数目标控制在 100～250 帧之间，上限为 250 帧。以当前十集短剧的统计为例，scene 级采样约得到 179 帧，加入最多 15 帧人物角色证据并去重后，预计仍在约 180～190 帧的范围内。若当前选取的剧集窗口去重后不足 100 帧，则继续按照 episode 顺序读取后续集数并执行相同的 scene 采样与人物帧补充策略，直到达到 100 帧或已经没有更多集数可用；达到 250 帧后停止继续加入视觉帧。

这里的 100～250 帧是剧集级训练输入的数据预算，不代表一次模型 forward 必须容纳全部帧。实际训练时仍应将剧集级证据池切分为符合上下文长度和显存预算的 episode 或 scene chunk，并在每个 chunk 内保留时间顺序、scene 覆盖和人物角色证据。
